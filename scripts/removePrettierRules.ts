...
namespaces.forEach((namespace) => {
  fs.readdirSync(path.resolve(__dirname, '../rules', namespace))
    .filter((ruleName) => fs.lstatSync(path.resolve(__dirname, '../rules', namespace, ruleName)).isDirectory())
    .forEach((ruleName) => {
      if (typeof prettierRules[namespace][`${RULE_PREFIX_MAP[namespace]}${ruleName}`] !== 'undefined') {
        const rulePath = path.resolve(__dirname, '../rules', namespace, ruleName);
        rimrafSync(rulePath);
